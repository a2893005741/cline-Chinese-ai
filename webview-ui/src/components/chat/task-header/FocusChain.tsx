...
const TODO_LIST_LABEL = "待办列表"
const NEW_STEPS_MESSAGE = "如果继续任务，将生成新的步骤"
const CLICK_TO_EDIT_TITLE = "点击在文件中编辑待办列表"
...
export const FocusChain: React.FC<FocusChainProps> = memo(
	({ currentTaskItemId, lastProgressMessageText, showPlaceholderWhenEmpty }) => {
		...
		if (!todoInfo) {
			...
			return (
				<div
					...
					<span className="text-sm text-foreground/80 truncate">待办事项</span>
				</div>
			)
		}
		...
		return (
			<div
				aria-label={isExpanded ? "收起专注链" : "展开专注链"}
